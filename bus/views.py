...
def addseat(request):
    if request.method == 'POST':
        ...
        data = seat()
        data.seatnumber = request.POST.get('seatnumber')
        data.bus_id = request.POST.get('bus_id')
        bus_data = busdetails.objects.get(id=data.bus_id)
        data.bus_name = bus_data.bustitle
        data.seatstatus = request.POST.get('seatstatus')
        data.save()
...
def busview(request):
    ...
    print('this is bus+id.......  :  ', bus)
    return render(request,'viewbus.html',{'res':sel,'bus':bus})
